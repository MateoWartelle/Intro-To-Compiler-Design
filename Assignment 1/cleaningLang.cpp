...
public:
    ...
    bool isAtEnd()
    const
    throw() { return (index_ >= input_.length()); }
...
void parseWashVerb(TokenStream &tokenizer
    ...
    if ((peek == WASH_KEYWORD_SYM) ||
        (peek == SCRUB_KEYWORD_SYM)) {
...
void parseWashItem(TokenStream &tokenizer
    ...
    // YOUR CODE HERE
    symbol_ty peek = tokenizer.peek();
    if ((peek == DISHES_KEYWORD_SYM) ||
        (peek == POTS_KEYWORD_SYM) ||
        (peek == WINDOWS_KEYWORD_SYM) ||
        (peek == SINK_KEYWORD_SYM) ||
        (peek == TOILET_KEYWORD_SYM) ||
        (peek == SHOWER_KEYWORD_SYM)) {
        delete(tokenizer.advance());
    }
    //  III.  Finished:
}
...
void parseWashList(TokenStream &tokenizer
    ...
    if ((peek == DISHES_KEYWORD_SYM) ||
        ...
        (peek == TOILET_KEYWORD_SYM) ||
        (peek == SHOWER_KEYWORD_SYM)) {
        parseWashItem(tokenizer);
        parseWashList(tokenizer);
    }
...
void parseRoomItem(TokenStream &tokenizer
    ...
    if ((peek == HALL_KEYWORD_SYM) ||
        (peek == FOYER_KEYWORD_SYM) ||
        (peek == BEDROOM_KEYWORD_SYM) ||
        (peek == KITCHEN_KEYWORD_SYM) ||
        (peek == BATHROOM_KEYWORD_SYM)) {
        delete(tokenizer.advance());
        //  III.  Finished:
    }
}
...
void parseS(TokenStream &tokenizer
    ...
    if (peek == END_OF_INPUT_SYM) {
        return;
    } else if ((peek == WASH_KEYWORD_SYM) || (peek == SCRUB_KEYWORD_SYM)) {
        parseWashSentence(tokenizer);
        parseS(tokenizer);
    } else if ((peek == VACUUM_KEYWORD_SYM) ||
               (peek == SWEEP_KEYWORD_SYM) ||
               (peek == MOP_KEYWORD_SYM)
            ) {
